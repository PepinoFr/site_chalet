...
class Estatik
{
     ...
     * @var string
     */
    protected static $_version = '3.11.8';
    ...
    public static function coupon_notice() {
        $curtime = time();
	    $is_black_friday = strtotime( '2021-11-24 00:00:00' ) <= $curtime && $curtime <= strtotime( '2021-11-30 23:59:59' );
	    $is_banner_active = false;

		if ( $is_black_friday ) {
			$endtime = strtotime( '2021-11-30 23:59:59' );

			if ( function_exists( 'es_get_agent' ) ) {
				if ( ! function_exists( 'es_mls_get_apis_list' ) ) {
					$banner = 'banner-black-friday-pro.php';
					$is_banner_active = true;
				}
			} else {
				$banner = 'banner-black-friday-simple.php';
				$is_banner_active = true;
			}

		} else {
			// Default banner for simple plugin (disabled for now)
...
//			$banner = 'simple.php';
//
//			$is_banner_active = $minutes > 0;
		}

	    if ( $is_banner_active && ! empty( $banner ) ) {
		    $diff = $endtime - $curtime;
		    $days = floor( $diff / 60 / 60 / 24 );
		    $hours = floor( $diff / 60 / 60 );
		    $minutes = floor( $diff / 60 );
    ...
    public static function register_js_variables()
    {
        ...
        return apply_filters( 'es_global_js_variables', array(
            ...
            'settings' => array(
	            ...
	            'is_lightbox_disabled' => $es_settings->is_lightbox_disabled
            ),
            'widgets' => array(
                'search' => array(
                    'initPriority' => array(
