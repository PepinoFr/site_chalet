...
//                $query_args['order'] = 'DESC';
			    ...
			    if ( in_array( $tax_name, $taxonomies ) && taxonomy_exists( $tax_name ) ) {
				    if ( ! empty( $value ) ) {
					    if ( $tax_name == 'es_labels' ) {
					    	...
					    	if ( is_string( $value ) ) {
							    $value = explode( $atts['fields_delimiter'], $value );
						    }
					        ...
					        $tax_values = is_string( $value ) ? array_map( 'trim', explode( $atts['fields_delimiter'], $value ) ) : $value;
			    ...
			    if ( ! empty( $field_info['fbuilder'] ) ) {
				    if ( ! empty( $value ) ) {
					    if ( ! empty( $field_info['values'] ) ) {
					    	...
					    	if ( is_string( $value ) ) {
							    $value = array_map( 'trim', explode( $atts['fields_delimiter'], $value ) );
						    }
						    ...
						    $field_values = array_map( 'trim', explode( $atts['fields_delimiter'], $value ) );
						    $query_args['meta_query'][ $key ]['relation'] = 'OR';
        ...
        if ( ! empty( $atts['prop_id'] ) ) {
            if ( is_array( $atts['prop_id'] ) ) {
                ...
                $query_args['post__in'] = array_map( 'trim', explode( $atts['fields_delimiter'], $atts['prop_id'] ) );
            }
