...
//***
        ...
        if (isset($_GET['page']) AND $_GET['page'] == 'woobe') {
            ...
            add_action('admin_notices', function () {
                ...
                if (!get_user_meta($user_id, 'woobe_notice_dismissed')) {
                    echo '<div class="notice notice-warning"><p>' . sprintf(esc_html__('If you not familiar with the plugin, firstly %s please', 'woo-bulk-editor'), WOOBE_HELPER::draw_link(array(
        ...
        add_action('admin_menu', function () {
            add_submenu_page('edit.php?post_type=product', 'BEAR ' . esc_html__('Bulk Editor', 'woo-bulk-editor'), 'BEAR ' . esc_html__('Bulk Editor', 'woo-bulk-editor'), 'manage_woocommerce', 'woobe', function () {
        ...
        add_action('admin_bar_menu', function ($wp_admin_bar) {
            ...
            if ($show) {
                $args = array(
                    'id' => 'woobe-btn',
                    'title' => 'BEAR ' . esc_html__('Bulk Editor', 'woo-bulk-editor'),
...
//***
    ...
    public function plugin_action_links($links) {
        ...
        $buttons = array(
            '<a href="' . admin_url('edit.php?post_type=product&page=woobe') . '">' . esc_html__('Products Editor', 'woo-bulk-editor') . '</a>',
            '<a target="_blank" href="https://bulk-editor.com/"><span class="icon-book"></span>&nbsp;' . esc_html__('Documentation', 'woo-bulk-editor') . '</a>'
        );
    ...
    public function admin_enqueue_scripts() {
        if (isset($_GET['page']) AND $_GET['page'] == 'woobe') {
            ...
            <script>
                var lang = {
                    move: "<?php echo esc_html__('move', 'woo-bulk-editor') ?>",
                    search: "<?php echo esc_html__('Search', 'woo-bulk-editor') ?>",
                    rest_failed: "<?php echo esc_html__('Failed', 'woo-bulk-editor') ?>",
                    error: "<?php echo esc_html__('Error', 'woo-bulk-editor') ?>",
                    delete: "<?php echo esc_html__('delete', 'woo-bulk-editor') ?>",
                    ignore: "<?php echo esc_html__('ignore', 'woo-bulk-editor') ?>",
                    no_deletable: "<?php echo esc_html__('This is not deletable!', 'woo-bulk-editor') ?>",
                    no_items: "<?php echo esc_html__('no items', 'woo-bulk-editor') ?>",
                    none: "<?php echo esc_html__('none', 'woo-bulk-editor') ?>",
                    no_data: "<?php echo esc_html__('no data', 'woo-bulk-editor') ?>",
                    loading: "<?php echo esc_html__('Loading', 'woo-bulk-editor') ?> ...",
                    loaded: "<?php echo esc_html__('Loaded', 'woo-bulk-editor') ?>.",
                    saved: "<?php echo esc_html__('Saved', 'woo-bulk-editor') ?>.",
                    saving: "<?php echo esc_html__('Saving', 'woo-bulk-editor') ?> ...",
                    apply: "<?php echo esc_html__('Apply', 'woo-bulk-editor') ?>",
                    cancel: "<?php echo esc_html__('Cancel', 'woo-bulk-editor') ?>",
                    canceled: "<?php echo esc_html__('Canceled', 'woo-bulk-editor') ?>",
                    sure: "<?php echo esc_html__('Sure?', 'woo-bulk-editor') ?>",
                    creating: "<?php echo esc_html__('Creating', 'woo-bulk-editor') ?> ...",
                    created: "<?php echo esc_html__('Created!', 'woo-bulk-editor') ?>",
                    duplicating: "<?php echo esc_html__('Duplicating', 'woo-bulk-editor') ?> ...",
                    duplicated: "<?php echo esc_html__('Duplicated!', 'woo-bulk-editor') ?>",
                    deleting: "<?php echo esc_html__('Deleting', 'woo-bulk-editor') ?> ...",
                    deleted: "<?php echo esc_html__('Deleted!', 'woo-bulk-editor') ?>",
                    reseting: "<?php echo esc_html__('Reseting', 'woo-bulk-editor') ?> ...",
                    reseted: "<?php echo esc_html__('Reseted!', 'woo-bulk-editor') ?>",
                    upload_image: "<?php echo esc_html__('Upload image', 'woo-bulk-editor') ?>",
                    upload_images: "<?php echo esc_html__('Upload images', 'woo-bulk-editor') ?>",
                    select_all: "<?php echo esc_html__('Select all', 'woo-bulk-editor') ?>",
                    deselect_all: "<?php echo esc_html__('Deselect all', 'woo-bulk-editor') ?>",
                    upload_file: "<?php echo esc_html__('Upload file', 'woo-bulk-editor') ?>",
                    fill_up_data: "<?php echo esc_html__('Fill up the data please!', 'woo-bulk-editor') ?>",
                    enter_duplicate_count: "<?php echo esc_html__('Enter how many time duplicate selected product(s)!', 'woo-bulk-editor') ?>",
                    enter_new_count: "<?php echo esc_html__('Enter how many new product(s) to create!', 'woo-bulk-editor') ?>",
                    search_input_placeholder: "<?php echo esc_html__('Text search by title or SKU', 'woo-bulk-editor') ?>",
                    show_panel: "<?php esc_html_e('Show: Filters/Bulk Edit/Export', 'woo-bulk-editor') ?>",
                    close_panel: "<?php esc_html_e('Hide: Filters/Bulk Edit/Export', 'woo-bulk-editor') ?>",
                    per_page: "<?php esc_html_e('Per page', 'woo-bulk-editor') ?>",
                    color_picker_col: "<?php esc_html_e('Select background color', 'woo-bulk-editor') ?>",
                    color_picker_txt: "<?php esc_html_e('Select text color', 'woo-bulk-editor') ?>",
                    sEmptyTable: "<?php esc_html_e('No data available in the table', 'woo-bulk-editor') ?>",
                    sInfo: "<?php esc_html_e('Showing _START_ to _END_ of _TOTAL_ entries', 'woo-bulk-editor') ?>",
                    sInfoEmpty: "<?php esc_html_e('Showing 0 to 0 of 0 entries', 'woo-bulk-editor') ?>",
                    sInfoFiltered: "<?php esc_html_e('(filtered from _MAX_ total entries)', 'woo-bulk-editor') ?>",
                    sLoadingRecords: "<?php esc_html_e('Loading...', 'woo-bulk-editor') ?>",
                    sProcessing: "<?php esc_html_e('Processing...', 'woo-bulk-editor') ?>",
                    sZeroRecords: "<?php esc_html_e('No matching records found', 'woo-bulk-editor') ?>",
                    sFirst: "<?php esc_html_e('First', 'woo-bulk-editor') ?>",
                    sLast: "<?php esc_html_e('Last', 'woo-bulk-editor') ?>",
                    sNext: "<?php esc_html_e('Next', 'woo-bulk-editor') ?>",
                    sPrevious: "<?php esc_html_e('Previous', 'woo-bulk-editor') ?>",
                    action_state_1: "<?php esc_html_e('all the products on the site', 'woo-bulk-editor') ?>",
                    action_state_2: "<?php esc_html_e('the filtered products. To remove the products filtering press reset button on the tools panel below', 'woo-bulk-editor') ?>",
                    action_state_31: "<?php esc_html_e('the selected products (variations)', 'woo-bulk-editor') ?>",
                    action_state_32: "<?php esc_html_e('You can reset selection of the products by its reset button on the panel of the editor OR uncheck them manually!', 'woo-bulk-editor') ?>",
                    term_maybe_exist: "<?php esc_html_e('Maybe term(s) with such name(s) already exists!', 'woo-bulk-editor') ?>",
                    free_ver_profiles: "<?php esc_html_e('In FREE version of the plugin you can create one profile only!', 'woo-bulk-editor') ?>",
                    append_sub_item: "<?php esc_html_e('append sub item', 'woo-bulk-editor') ?>",
                    is_deactivated_in_free: "<?php esc_html_e('This field is deactivated in FREE version for bulk edit!', 'woo-bulk-editor') ?>",
                    checked_products: "<?php esc_html_e('Products checked', 'woo-bulk-editor') ?>"
                };
...
//***
            ...
            wp_enqueue_script('woobe_autocomplete', WOOBE_ASSETS_LINK . 'js/easy-autocomplete/jquery.easy-autocomplete.min.js', array('jquery'), WOOBE_VERSION);
...
//buttons: edit + view
            $row[] = WOOBE_HELPER::draw_link(array(
                        ...
                        'title_attr' => esc_html__('View the product on the site front', 'woo-bulk-editor')
                    )) . '&nbsp;' . WOOBE_HELPER::draw_link(array(
                        'title' => '&#xea25;',
                        'href' => get_admin_url() . 'post.php?post=' . $p['ID'] . '&action=edit',
                        'target' => '_blank',
                        'class' => 'button button-primary button-small button-small-2',
                        'title_attr' => esc_html__('Editing of the product on its page', 'woo-bulk-editor')
            ));
        } else {
            $row[] = '';
        }
...
//ajax
    public function get_post_field() {
        echo $this->products->get_post_field(intval($_REQUEST['product_id']), sanitize_key($_REQUEST['field']), (isset($_REQUEST['post_parent']) ? intval($_REQUEST['post_parent']) : 0));
        exit;
    }
...
//http://easyautocomplete.com/guide
    ...
    public function woobe_create_new_product() {
        ...
        $wp_rest_request->set_body_params(array(
            'name' => esc_html__('New Product', 'woo-bulk-editor'),
            'description' => '',
...
//using for products selection
        ...
        if ($field_key === 'ID') {
            ...
            if ($product->get_type() === 'variable') {
                ...
                $title = esc_html__('see the product on the site OR select products variations if they are shown', 'woo-bulk-editor');
            }
...
//do not onit it exept of one woobe page and its ajax requests
    ...
    public function add_statuses($statuses) {
        return $statuses + [
            'future' => esc_html__('Scheduled', 'woo-bulk-editor')
        ];
    }
        ...
        add_action('admin_notices', function () use ($slug) {
            ...
            <div class="notice notice-info" id="pn_<?php echo $slug ?>_ask_favour" style="position: relative;">
                <button onclick="javascript: pn_<?php echo $slug ?>_dismiss_review(1);
                                    void(0);" title="<?php esc_html_e('Later', 'woo-bulk-editor'); ?>" class="notice-dismiss"></button>
                <div id="pn_<?php echo $slug ?>_review_suggestion">
                    <p><?php esc_html_e('Hi! Are you enjoying using BEAR - WooCommerce Bulk Editor and Products Manager Professional?', 'woo-bulk-editor'); ?></p>
                    <p><a href="javascript: pn_<?php echo $slug ?>_set_review(1); void(0);"><?php esc_html_e('Yes, I love it', 'woo-bulk-editor'); ?></a> 🙂 | <a href="javascript: pn_<?php echo $slug ?>_set_review(0); void(0);"><?php esc_html_e('Not really...', 'woo-bulk-editor'); ?></a></p>
                </div>

                <div id="pn_<?php echo $slug ?>_review_yes" style="display: none;">
                    <p><?php printf(esc_html__('That\'s awesome! Could you please do us a BIG favor and give it a 5-star rating on %s to help us spread the word and boost our motivation?', 'woo-bulk-editor'), $on) ?></p>
                    <p style="font-weight: bold;">~ PluginUs.NET developers team</p>
                    <p>
                        <a href="<?php echo $link ?>" style="display: inline-block; margin-right: 10px;" onclick="pn_<?php echo $slug ?>_dismiss_review(2)" target="_blank"><?php esc_html_e('Okay, you deserve it', 'woo-bulk-editor'); ?></a>
                        <a href="javascript: pn_<?php echo $slug ?>_dismiss_review(1); void(0);" style="display: inline-block; margin-right: 10px;"><?php esc_html_e('Nope, maybe later', 'woo-bulk-editor'); ?></a>
                        <a href="javascript: pn_<?php echo $slug ?>_dismiss_review(2); void(0);"><?php esc_html_e('I already did', 'woo-bulk-editor'); ?></a>
                    </p>
                </div>

                <div id="pn_<?php echo $slug ?>_review_no" style="display: none;">
                    <p><?php esc_html_e('We are sorry to hear you aren\'t enjoying BEAR. We would love a chance to improve it. Could you take a minute and let us know what we can do better?', 'woo-bulk-editor'); ?></p>
                    <p>
                        <a href="https://pluginus.net/contact-us/" onclick="pn_<?php echo $slug ?>_dismiss_review(2)" target="_blank"><?php esc_html_e('Give Feedback', 'woo-bulk-editor'); ?></a>&nbsp;
                        |&nbsp;<a href="javascript: pn_<?php echo $slug ?>_dismiss_review(2); void(0);"><?php esc_html_e('No thanks', 'woo-bulk-editor'); ?></a>
                    </p>
                </div>
